...
$sql = "SELECT * FROM event_forum_categories WHERE cat_id='$catID'";
...
else
{
    ...
    else
    {
        ...
        {
            echo '<h2>Topics in the ′' . $row['cat_name'] . '′ category</h2>';
        }
     
        //do a query for the topics
        $sql = "SELECT * FROM event_forum_topics WHERE topic_cat='$catID'";
        $result = mysqli_query($link, $sql);
                    ...
                    echo '<tr>';
                        echo '<td class="leftpart">';
                            echo '<div id="'.$row['topic_id'].'">
                                    <h3 class="topic-title"><a href="eventforumposts.php?id=' . $row['topic_id'] . '">' . $row['topic_subject'] . '</a><h3>
                                </div>
