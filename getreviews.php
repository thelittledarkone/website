...
if ($edition_id != 0 && $edition_id != NULL){
    $sql = "SELECT * FROM game_reviews WHERE user_id != '$user_id' AND edition_id='$edition_id' ORDER BY date DESC LIMIT $start_from, $results_per_page";
}else{
    $sql = "SELECT * FROM game_reviews WHERE user_id != '$user_id' AND game_id='$game_id' ORDER BY date DESC LIMIT $start_from, $results_per_page";
}
...
if($result){
    if(mysqli_num_rows($result)>0){
        ...
        if($isEmpty == 0){
            echo "<div class='alert alert-warning noResults'>No reviews for this game yet!</div>";
        }
    }else{
...
if ($edition_id != 0 || $edition_id != NULL){
    $sql4 = "SELECT COUNT(*) AS count FROM game_reviews WHERE user_id != '$user_id' AND edition_id='$edition_id'";
}else{
    $sql4 = "SELECT COUNT(*) AS count FROM game_reviews WHERE user_id != '$user_id' AND game_id='$game_id'";
}
$result4 = mysqli_query($link, $sql4);
$resultCount = mysqli_fetch_assoc($result4)['count'];
        
$total_pages = ceil($resultCount / $results_per_page);

echo "<div class='resultsPages'>";
for ($i=1; $i<=$total_pages; $i++) { 
    if($edition_id == 0 || $edition_id == NULL){
        echo "<a href='userreviews.php?game_name=$game_name&edition=0&page=$i'>$i</a>";
    }else{
        echo "<a href='userreviews.php?game_id=$game_id&edition=$edition_id&page=$i'>$i</a>";
    }
