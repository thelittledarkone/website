...
if($_SESSION['uri'] === "/eventsnearme.php"){
    $sql1 .= " AND user_id != '$user_id' ORDER BY date_added DESC";
}elseif($_SESSION['uri'] === "/gameevents.php?game=".$game_id."&edition=".$edition_id){
    if($edition_id == 0){
        $sql1 .= " AND event_game='$game_id' AND user_id != '$user_id' ORDER BY date_added DESC";
    }else{
        $sql1 .= " AND event_game='$game_id' AND game_edition='$edition_id' AND user_id != '$user_id' ORDER BY date_added DESC";
    }
    
}else{
    $sql1 .= " AND user_id != '$user_id' ORDER BY date_added DESC LIMIT 5";
}
...
//    echo "<div class='alert alert-info noResults'>There are no events near you!</div>"; exit;
//}

echo "<div id='eventsNearMeResults'>";
...
//                $eventaddress = $row1['address'];
//                $eventaddress2 = $row1['address2'];
//                $eventdistrict = $row1['district'];
                $eventcity = $row1['city'];
//                $eventpostcode = $row1['postal_code'];
                $eventname = $row1['event_name'];
                
                $eventgame = $row1['event_game'];
                //Get game details
                ...
                $eventtournseats = $row1['tournseatsavailable'];
                $attendees = $row1['attendees'];
                $tournseatsleft = $eventtournseats - $attendees;
        ...
        if(($eventType == 'o' || $eventseats > 0 || $tournseatsleft > 0) && ($player1 != $user_id && $player2 != $user_id && $player3 != $user_id && $player4 != $user_id && $player5 != $user_id && $player6 != $user_id && $player7 != $user_id && $player8 != $user_id && $openAttend != '1' && $tournAttend != '1')){
            //Print Event Card
            echo "
        <div class='individualEvent'>
            <div class='row mainContent'>
                <div class='col-sm-3'>
                    ...
                    echo "</a></div>
                    <a href='profilepage.php?username=$username'><div class='organiser'>$username</div></a>
                </div>
                <div class='col-sm-7 detailsContainer nearDetailsContainer'>
                        <table class='table table-condensed'>
                            ...
                            <tr>
                                ...
                                <td class='col-xs-9'>";
                            if($gameedition == 0 || $gameedition == NULL){
                                echo "<a href='gameprofile.php?game_name=$game_name&edition=0'><span>$game_name</span></a>";
                            }else{
                    ...
                    if($eventType == 'o' || $eventType == 't'){
                        echo "<tr>
                                <td class='col-xs-3'>Attendees:</td>
                                <td class='col-xs-9'>
                                    <div>$attendees</div>
                                </td>
                            </tr>";
                ...
                <div class='col-sm-2 priceContainer'>
                    <div class='priceStats'>
                        ...
                        <div class='seatDetails'>Open Event!</div>
                                            ...
                                            echo "Tournament</div>
                                                <div>Event with</div>
                        <div class='seatDetails'>$tournseatsleft/$eventtournseats seats left!</div>
                    </div>";
                                        }else{
                                            echo "Limited</div>
                                                <div>Event with</div>
                        <div class='seatDetails'>$eventseats seats left!</div>
                    </div>";
                                        }
            echo "</div>
            </div> 
        ...
        if($isEmpty == 0){
            echo "<div class='alert alert-info noResults'>Try expanding your search radius to more than $radius miles, to see more events!</div>";
        }
    }else{
